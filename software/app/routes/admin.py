...
def add_card():
    ...
    try:
        ...
        if ticket_type == "monthly":
            # Lấy mức phí tháng hiện tại từ settings
            ...
            # Ghi doanh thu cho tháng đầu tiên
            month_label = created_at_dt.strftime("%Y-%m")
            conn.execute(
                "INSERT INTO monthly_payments (card_id, month, amount, paid_at) VALUES (?, ?, ?, ?)",
                (card_id, month_label, monthly_fee, created_at),
            )
        conn.commit()
...
def settings():
    ...
    if request.method == "POST":
        try:
            # 1. Xử lý chuỗi nhập vào: Xóa dấu chấm/phẩy (VD: "1.200.000" -> "1200000")
            raw_fee_hour = request.form["fee_per_hour"].replace('.', '').replace(',', '')
            raw_fee_month = request.form["monthly_fee"].replace('.', '').replace(',', '')
            
            fee_per_hour = int(raw_fee_hour)
            monthly_fee = int(raw_fee_month)

            # 2. Validate: Kiểm tra bội số của 1.000
            if fee_per_hour % 1000 != 0 or monthly_fee % 1000 != 0:
                flash("Giá vé phải là bội số của 1.000 VNĐ (ví dụ: 3000, 5000...)", "danger")
            else:
